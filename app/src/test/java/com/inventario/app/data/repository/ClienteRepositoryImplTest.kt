...
class ClienteRepositoryImplTest {
    ...
    @Before
    fun setup() {
        repository = ClienteRepositoryImpl(clienteApi, workspaceDataStore)
    }
    ...
    fun `getClientes returns mapped list`() = runTest {
        ...
        coEvery { clienteApi.getClientes("org", "ws") } returns listOf(sampleClienteDto)
    ...
    fun `getClienteById returns success`() = runTest {
        coEvery { workspaceDataStore.orgSlug } returns flowOf("org")
        coEvery { workspaceDataStore.wsSlug } returns flowOf("ws")
        coEvery { clienteApi.getClienteById("org", "ws", 1) } returns sampleClienteDto
    ...
    fun `createCliente returns success`() = runTest {
        coEvery { workspaceDataStore.orgSlug } returns flowOf("org")
        coEvery { workspaceDataStore.wsSlug } returns flowOf("ws")
        coEvery { clienteApi.createCliente(any(), any(), any()) } returns sampleClienteDto
    ...
    fun `createCliente builds correct request`() = runTest {
        ...
        coVerify {
            clienteApi.createCliente("org", "ws", any())
        }
    }
    ...
    fun `createCliente returns NetworkError on exception`() = runTest {
        coEvery { workspaceDataStore.orgSlug } returns flowOf("org")
        coEvery { workspaceDataStore.wsSlug } returns flowOf("ws")
        coEvery { clienteApi.createCliente(any(), any(), any()) } throws RuntimeException("error")
    ...
    fun `updateCliente returns success`() = runTest {
        coEvery { workspaceDataStore.orgSlug } returns flowOf("org")
        coEvery { workspaceDataStore.wsSlug } returns flowOf("ws")
        coEvery { clienteApi.updateCliente(any(), any(), any(), any()) } returns sampleClienteDto
    ...
    fun `updateCliente returns NotFound when API returns 404`() = runTest {
        ...
        coEvery { clienteApi.updateCliente(any(), any(), any(), any()) } throws responseException
